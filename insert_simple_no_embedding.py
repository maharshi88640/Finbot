#!/usr/bin/env python3
"""
Insert Scraped Documents to Database (No Embedding)
Loads the branch-specific scraped documents and inserts them into the database without embeddings
"""
...
def insert_branch_documents():
    """Insert the branch-specific scraped documents into database without embeddings"""
    ...
    # Prepare documents for database insertion (without embedding)
    docs_for_insertion = []
    success_count = 0
    ...
    for i, doc in enumerate(documents, 1):
        try:
            # Prepare document without embedding
            doc_data = {
                'gr_no': doc.get('gr_no', ''),
                'date': doc.get('date', ''),
    ...
    for batch_num in range(total_batches):
        ...
        try:
            # Direct insert to Supabase table without using the DatabaseManager method
